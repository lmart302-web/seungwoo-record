...
function drawYearlyWeight() {
  ...
  for (let month = 0; month < 12; month++) {
    ...
    if (elements.yearWeightList) {
      const li = document.createElement("li");
      li.innerHTML = `
        <span class="record-date" style="color: #666666;">${month + 1}월</span>
        <span class="record-colon">:</span>
        <span class="record-value" style="color: #222222; font-weight: 600;">${averageValue.toFixed(1)}kg</span>
        <span class="record-badge" style="font-weight: bold; color: ${badgeColor}; margin-left: 4px;">${badgeText}</span>
      `;
...
function drawAllWeight() {
  ...
  monthlyRecords.forEach((item) => {
    ...
    if (elements.allWeightList) {
      ...
      const li = document.createElement("li");
      li.innerHTML = `
        <span class="record-date" style="color: #666666;">${item.month + 1}월</span>
        <span class="record-colon">:</span>
        <span class="record-value" style="color: #222222; font-weight: 600;">${item.average.toFixed(1)}kg</span>
        <span class="record-badge" style="font-weight: bold; color: ${badgeColor}; margin-left: 4px;">${badgeText}</span>
      `;
