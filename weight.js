...
// PC에서도 확실히 인지할 수 있는 애니메이션 설정 (0.8초 동안 아래에서 위로/좌에서 우로 차례로 등장)
// 애니메이션 설정: 왼쪽에서 오른쪽으로 선이 그려지면서 데이터 포인트가 등장
const ANIMATION_CONFIG = {
  x: {
    type: 'number',
    easing: 'linear',
    duration: 1500, // 전체 선이 그려지는 시간 (1.5초)
    from: NaN, // 시작 지점
    delay(ctx) {
      ...
      ctx.xStarted = true;
      // 인덱스에 따라 약간의 딜레이를 주어 순차적으로 그려지도록 설정
